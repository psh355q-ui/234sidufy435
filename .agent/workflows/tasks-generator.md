---
description: TDD 워크플로우와 Git Worktree 규칙이 적용된 TASKS.md 자동 생성
---
...
- 명령어: `/tasks-generator`
- 키워드: "TASKS.md 만들어줘", "태스크 생성"
- 또는 `/socrates` 완료 후 자동 호출
...
```
.agent/scripts/tasks-generator/references/tasks-rules.md
```
...
.agent/rules/planning-standards.md
```
...
"예" 선택 시 → `/project-bootstrap` 워크플로우 호출

---

## 핵심 규칙 요약

상세 규칙은 `.agent/rules/tdd-workflow.md`와 `.agent/rules/git-conventions.md`에 있습니다.
...
- `.agent/workflows/socrates.md`: 기획 문서 생성 (자동으로 이 워크플로우 호출)
- `.agent/workflows/project-bootstrap.md`: 프로젝트 구조 생성

## 관련 규칙

- `.agent/rules/tdd-workflow.md`: TDD 사이클 규칙
- `.agent/rules/git-conventions.md`: Git Worktree 전략
- `.agent/rules/planning-standards.md`: TASKS.md 작성 표준
